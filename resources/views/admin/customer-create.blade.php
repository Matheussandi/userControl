            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="post" action="{{ route('customers.store') }}" class="mt-6 space-y-6">
                        ...
                        <div>
                            <x-text-input name="user_id" type="hidden" class="mt-1 block w-full" 
                            autocomplete="current-password" value="{{ Auth::user()->id }}" />
                        </div>
                
                        <div>
                            <x-input-label for="name" :value="__('Nome')" />
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" autocomplete="name" autofocus required/>
                        </div>
